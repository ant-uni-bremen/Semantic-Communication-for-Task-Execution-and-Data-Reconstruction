...
class ResNet14Encoder(Model):
    def __init__(self, split_image_into, enc_out_dec_inp, *, activity_regularizer=None, trainable=True, dtype=None, autocast=True, name=None, **kwargs):
        ...
        # ResNet-14 follows this block sequence: (Conv → Residuals → Downsample)
        self._encoder = Sequential([
            ...
            ResidualBlock(filters=128, strides=1),

            # Downsampling to 8x8
            ResidualBlock(filters=256, strides=2),
            ResidualBlock(filters=256, strides=1),

            # Flatten and Dense
            layers.Flatten(),
            layers.Dense(self.enc_out_dec_inp // (self.split_image_into), activity_regularizer=regularizers.l2(0.001)),
        ], name='resnet14_encoder')
